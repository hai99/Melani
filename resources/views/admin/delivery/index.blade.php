...
@section('title','Quản lý hình thức giao hàng')
...
@if($rolePermission->contains('delivery-create'))
@section('search-add')
  <div class="panel-body">
    <form action="" method="POST" class="form-inline" role="form">
      <a href="" class="btn btn-success" id="deli_add">Thêm mới</a>
    </form>
  </div>
@stop()
@endif
      ...
      <div class="col-xs-12">
        <div class="box">
          <div class="box-header">
            <h3 class="box-title">Quản lý hình thức giao hàng</h3>
          </div>
        <!-- /.box-header -->
        <div class="box-body deli_icheck">
          ...
          <tbody>
            ...
            <tr>
              <td>
                <input type="checkbox" class="color_checkbox" value="{{ $deli->id }}">
              </td>
              <td>{{ $deli->name }}</td>
              <td>{{ $deli->fee }}</td>
              <td><span class="badge badge-success">{{ $deli->status?"Hiển thị" : "Ẩn" }}</span></td>
              <td>{{ date('d-m-Y',strtotime($deli->created_at)) }}</td>
              <td>
                @if($rolePermission->contains('delivery-edit'))
                <a href="" class="btn btn-success edit" id="{{ $deli->id }}"><i class="fa fa-edit"></i></a>
                @endif
                <input type="hidden" name="hidden_name" class="hidden_name" value="{{ $deli->name }}">
                @if($rolePermission->contains('delivery-delete'))
                <a href="" class="btn btn-danger delete" id="{{ $deli->id }}"><i class="fa fa-remove"></i></a>
                @endif
              </td>
            </tr>
            @endforeach
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="" method="POST" role="form" id="form_demo">
          ...
          <div class="form-group">
            <label for="" class="control-label">Phí</label>
...
@section('js')
<script>
  $('#deli_table').DataTable();
</script>
<script>
  $('#deli_add').on('click',function(event){
    event.preventDefault();
    $('.modal-title').text('Thêm mới hình thức giao hàng');
    ...
    $(document).on('click','.edit',function(event){
      ...
      $.ajax({
        ...
        success:function(html){
          $('.modal-title').text('Chỉnh sửa hình thức giao hàng');
          $('#form_demo').attr('id', 'form_edit');
          $('#form_add').attr('id', 'form_edit');
          $('#name').val(html.data.name);
          $('#fee').val(html.data.fee);
...
<script>
  $(function () {
    ...
    $('.deli_icheck input[type="checkbox"]').iCheck({
    ...
    $(".checkbox-toggle").click(function () {
      ...
      if (clicks) {
        ...
        $(".deli_icheck input[type='checkbox']").iCheck("uncheck");
        $(".fa", this).removeClass("fa-check-square-o").addClass('fa-square-o');
      } else {
        //Check all checkboxes
        $(".deli_icheck input[type='checkbox']").iCheck("check");
