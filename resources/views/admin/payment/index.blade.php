...
@if($rolePermission->contains('payment-create'))
@section('search-add')
  <div class="panel-body">
    <form action="" method="POST" class="form-inline" role="form">
      <a href="" class="btn btn-success" id="payment_add">Thêm mới</a>
    </form>
  </div>
@stop()
@endif
      ...
      <div class="col-xs-12">
        <div class="box">
          <div class="box-header">
            <h3 class="box-title">Quản lý hình thức thanh toán</h3>
          </div>
        <!-- /.box-header -->
        <div class="box-body pay_icheck">
          ...
          <tbody>
            ...
            <tr>
              <td>
                <input type="checkbox" class="pay_checkbox" value="{{ $pay->id }}">
              </td>
              <td>{{ $pay->name }}</td>
              <td><span class="badge badge-success">{{ $pay->status?"Hiển thị" : "Ẩn" }}</span></td>
              <td>{{ date('d-m-Y',strtotime($pay->created_at)) }}</td>
              <td>
                @if($rolePermission->contains('payment-edit'))
                <a href="" class="btn btn-success edit" id="{{ $pay->id }}"><i class="fa fa-edit"></i></a>
                @endif
                <input type="hidden" name="hidden_name" class="hidden_name" value="{{ $pay->name }}">
                @if($rolePermission->contains('payment-delete'))
                <a href="" class="btn btn-danger delete" id="{{ $pay->id }}"><i class="fa fa-remove"></i></a>
                @endif
              </td>
            </tr>
            @endforeach
...
<script>
  $('#payment_add').on('click',function(event){
    ...
    $('.modal-title').text('Thêm mới hình thức thanh toán');
...
<script>
  $(function () {
    ...
    $('.pay_icheck input[type="checkbox"]').iCheck({
    ...
    $(".checkbox-toggle").click(function () {
      ...
      if (clicks) {
        ...
        $(".pay_icheck input[type='checkbox']").iCheck("uncheck");
        $(".fa", this).removeClass("fa-check-square-o").addClass('fa-square-o');
      } else {
        //Check all checkboxes
        $(".pay_icheck input[type='checkbox']").iCheck("check");
